...
@Injectable()
export class BusinessGatewayService {
  private readonly businessUrl =
  ...
  private async parseResponse(res: Response) {
    ...
    if (!res.ok) {
      const body = contentType.includes('application/json')
        ...
        : await res.text();
      throw new Error(`Business service error (${res.status}): ${body}`);
    }
  ...
  private buildUrl(
    ...
    const url = new URL(`${this.businessUrl}${path}`);
  ...
  async get(path: string, clientId: string, query?: Record<string, string | undefined>) {
    ...
    try {
      ...
      throw new Error(
        `Business service unreachable: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  ...
  async post(path: string, clientId: string, body?: unknown, query?: Record<string, string | undefined>) {
    ...
    try {
      res = await this.fetchWithRetry(
        this.buildUrl(path, clientId, query),
        {
          method: 'POST',
          headers: {
