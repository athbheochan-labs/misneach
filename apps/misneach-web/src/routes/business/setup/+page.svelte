<script lang="ts">
  ...
  const ONBOARDING_CLIENT_ID_KEY = 'misneach_business_onboarding_client_id_v1';
  ...
  function getClientId() {
    ...
    const existing = localStorage.getItem(ONBOARDING_CLIENT_ID_KEY);
    if (existing) return existing;
    const created = typeof crypto !== 'undefined' && crypto.randomUUID ? crypto.randomUUID() : `biz_${Date.now()}`;
    localStorage.setItem(ONBOARDING_CLIENT_ID_KEY, created);
    return created;
  }

  async function onboardingFetch(path: string, init: RequestInit = {}) {
    const clientId = encodeURIComponent(getClientId());
    const join = path.includes('?') ? '&' : '?';
    return fetch(`/api/business${path}${join}clientId=${clientId}`, init);
  }
  ...
  async function saveStep1() {
    const response = await onboardingFetch('/onboarding/details', {
      method: 'POST',
      headers: {
  ...
  async function saveStep2() {
    const response = await onboardingFetch('/onboarding/staff', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
  ...
  async function applyPromo() {
    ...
    const response = await onboardingFetch('/onboarding/promo/validate', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
  ...
  async function handlePayment() {
    ...
    try {
      ...
      if (!promoApplied || totalCents > 0) {
        ...
        const intentResponse = await onboardingFetch('/onboarding/payment-intent', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
      ...
      const activateResponse = await onboardingFetch('/onboarding/activate', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  async function preload() {
    try {
      ...
      const onboardingResponse = await onboardingFetch('/onboarding', { cache: 'no-store' });
      ...
      if (onboardingResponse.ok) {
        ...
        if (currentStep >= 4) {
          const assetsResponse = await onboardingFetch('/onboarding/kit-assets', {
            cache: 'no-store',
          });
...
<svelte:head>
  <title>Misneach - Set up your business</title>
  <link
