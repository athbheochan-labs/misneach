  ...
  {#each goals as goal (goal.id)}
    <div class="rounded-xl border bg-white p-4 space-y-2">
      ...
      <div class="flex gap-2">
        <MisButton variant="unstyled" size="none" onclick={() => dispatch('checkoff', { id: goal.id })} className="rounded border px-2 py-1 text-xs">Check off</MisButton>
        {#if goal.status !== 'archived'}
          <MisButton variant="unstyled" size="none" onclick={() => dispatch('archive', { id: goal.id })} className="rounded border px-2 py-1 text-xs">Archive</MisButton>
        {/if}
      </div>
    </div>
  {/each}
