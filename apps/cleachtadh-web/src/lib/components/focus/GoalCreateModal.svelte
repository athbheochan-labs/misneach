  ...
  <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onclick={(e) => e.target === e.currentTarget && close()}>
    <div class="w-full max-w-lg space-y-4 rounded-xl bg-white p-6">
      ...
      <div class="space-y-2">
        ...
        <MisInput id="goal-title" bind:value={title} placeholder="e.g. Complete 5 speaking sessions" variant="unstyled" className="w-full rounded border px-3 py-2 text-sm" />
      </div>

      <div class="space-y-2">
        <label for="goal-description" class="text-sm font-medium text-gray-700">Description (optional)</label>
        <MisTextarea id="goal-description" bind:value={description} placeholder="Add context for this goal" variant="unstyled" className="w-full rounded border px-3 py-2 text-sm"></MisTextarea>
      </div>
      ...
      <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
        ...
        <div class="space-y-2">
          ...
          <MisSelect id="goal-activity" bind:value={activityType} variant="unstyled" className="w-full rounded border px-3 py-2 text-sm">
      ...
      <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
        ...
        <div class="space-y-2">
          ...
          <MisInput id="goal-due" type="date" bind:value={periodEnd} variant="unstyled" className="w-full rounded border px-3 py-2 text-sm" />
        </div>
      </div>

      <div class="space-y-2">
        <label for="goal-target-type" class="text-sm font-medium text-gray-700">Target metric</label>
        <MisSelect id="goal-target-type" bind:value={targetType} variant="unstyled" className="w-full rounded border px-3 py-2 text-sm">
      ...
      <div class="space-y-2">
        ...
        <MisInput id="goal-target-value" type="number" min="1" bind:value={targetValue} variant="unstyled" className="w-full rounded border px-3 py-2 text-sm" />
      ...
      <div class="flex justify-end gap-2 pt-2">
        <MisButton variant="unstyled" size="none" onclick={close} className="rounded border px-4 py-2">Cancel</MisButton>
        <MisButton variant="unstyled" size="none" onclick={submit} className="rounded bg-emerald-600 px-4 py-2 text-white">Create</MisButton>
      </div>
    </div>
  </div>
{/if}
